  ...
  <div class="bg-onboarding flex items-center justify-center p-4">
    ...
    <div class="card-area-elevated p-8 w-full max-w-md relative z-10 animate-fade-in-up shadow-premium">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-3xl font-bold text-area-gradient mb-2">AREA</h1>
        <p class="text-surface-500 dark:text-surface-400">Automate your workflow</p>
      </div>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div class="flex items-center justify-between">
          <label class="flex items-center cursor-pointer group">
            <input type="checkbox" class="w-4 h-4 text-primary-600 border-surface-300 rounded focus:ring-primary-500 focus:ring-offset-0" />
            <span class="ml-2 text-sm text-surface-600 group-hover:text-surface-800 transition-colors">Remember me</span>
          </label>
          <button 
            type="button"
        ...
        <button
          ...
          class="w-full flex items-center justify-center gap-3 bg-white border border-surface-200 text-surface-700 py-3 rounded-xl font-medium hover:bg-surface-50 hover:border-surface-300 transition-all shadow-soft hover:shadow-soft-lg"
        >
      ...
      <!-- 2FA Modal -->
      <div v-if="show2FAModal" class="fixed inset-0 bg-surface-900/60 backdrop-blur-sm flex items-center justify-center z-50 animate-fade-in">
        <div class="card-area-elevated p-8 w-full max-w-md mx-4 animate-scale-in">
          <div class="text-center mb-6">
            <div class="inline-flex items-center justify-center w-16 h-16 bg-gradient-to-br from-primary-600 to-primary-700 rounded-2xl shadow-primary mb-4">
              <Mail :size="32" class="text-white" />
            </div>
            <h2 class="text-2xl font-bold text-surface-900 mb-2">Enter Verification Code</h2>
            <p class="text-surface-500">We sent a 6-digit code to {{ email }}</p>
          </div>

          <form @submit.prevent="handleTwoFAVerification" class="space-y-6">
            <div>
              <label for="twoFACode" class="label">
                Verification Code
              </label>
              <input
                id="twoFACode"
                ...
                :class="[
                  'input-area text-center text-2xl font-mono tracking-widest',
                  errors.twoFACode && 'input-area-error'
                ]"
                placeholder="123456"
                required
              />
              <p v-if="errors.twoFACode" class="mt-2 text-sm text-red-500">
                {{ errors.twoFACode }}
              </p>
            </div>
            ...
            <button
              type="button"
              @click="cancel2FA"
              class="w-full text-surface-500 hover:text-surface-700 font-medium transition-colors"
            >
              Cancel
            </button>
          </form>
      ...
      <p class="text-center text-sm text-surface-500 mt-6">
        ...
        <router-link to="/signup" class="text-primary-600 font-semibold hover:text-primary-700 transition-colors">
          Sign up
        </router-link>
      </p>
    </div>
...
import { Mail, Lock, Eye, EyeOff, Workflow, ArrowLeft } from 'lucide-vue-next';
